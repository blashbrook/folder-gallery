...
- All gallery HTML/CSS/JS is generated by JavaScript at runtime. Do not look for static templates.
  - UI generator: bin/server-runner.js → function generateIndexHTML() writes .gallery-cache/index.html with embedded CSS/JS.
  - Thumbnails: bin/server-runner.js generates .gallery-cache/thumbnails/*.jpg (Sharp; FFmpeg optional).
  - CLI: bin/gallery.js manages background process, PID file, and reads .gallery-cache/server-info.json.
...
UI generation responsibilities:
- generateIndexHTML (bin/server-runner.js): Entire HTML document and inlined client logic (SSE wiring, masonry layout, modal, favorites, theme toggle, viewport reporting).
- broadcastToClients (bin/server-runner.js): Sends SSE events as lines of `data: <json>\n\n`.
- Thumbnail pipeline (bin/server-runner.js): Sharp-based image thumbs, FFmpeg-assisted video frames, fallbacks.
- CLI (bin/gallery.js): Background spawn, PID/server-info management, delete/cleanup/scan commands.
