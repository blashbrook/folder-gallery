...
const os = require('os');
const { isTagCommandAvailable, readFinderTags, writeFinderTags } = require(path.join(__dirname, '..', 'macos-tags.js'));

// ESM-only 'open' support via dynamic import to avoid require() ESM error

// Track macOS tag command availability
let tagCommandAvailable = false;
...
const MAX_VIDEO_WORKERS = Number.isFinite(parseInt(config.maxWorkers)) && parseInt(config.maxWorkers) > 0 
    ? Math.min(parseInt(config.maxWorkers), 4) 
    : 2;
const FFMPEG_TIMEOUT = Number.isFinite(parseInt(config.ffmpegTimeoutMs)) && parseInt(config.ffmpegTimeoutMs) > 0 
    ? parseInt(config.ffmpegTimeoutMs) 
    : 20000;
const FFPROBE_TIMEOUT = Math.min(FFMPEG_TIMEOUT, 15000);
const DISABLE_VIDEO_THUMBS = !!config.disableVideoThumbs;
...
// Helpers for video frame extraction using ffprobe/ffmpeg
    ...
    try {
        const { stdout } = await new Promise((resolve, reject) => {
            execFile('ffprobe', [
                ...
                videoPath
            ], { timeout: FFPROBE_TIMEOUT }, (err, stdout, stderr) => {
...
// Simple semaphore to cap concurrent video frame extractions
let __videoWorkers = MAX_VIDEO_WORKERS;
const __videoQueue = [];
function acquireVideoWorker() {
    if (__videoWorkers > 0) { __videoWorkers--; return Promise.resolve(); }
    return new Promise(res => __videoQueue.push(res));
}
function releaseVideoWorker() {
    __videoWorkers++;
    if (__videoQueue.length) { __videoWorkers--; __videoQueue.shift()(); }
}

async function extractVideoFrame(videoPath, outPath, timeSec) {
    return new Promise((resolve) => {
        execFile('ffmpeg', [
            '-ss', String(Math.max(0, timeSec || 1)),
            '-i', videoPath,
            '-frames:v', '1',
            '-q:v', '3',
            '-y', outPath
        ], { timeout: FFMPEG_TIMEOUT }, (err) => {
            resolve(!err);
        });
    });
}

async function generateVideoThumbnailFromFrame(videoPath, thumbnailPath) {
    if (DISABLE_VIDEO_THUMBS) return false;
    await acquireVideoWorker();
    try {
        // Ensure a temp dir inside cache
        const tmpDir = path.join(GALLERY_CACHE_DIR, 'tmp');
        await fs.mkdir(tmpDir, { recursive: true });
        const tmpOut = path.join(tmpDir, `frame-${Date.now()}-${Math.random().toString(36).slice(2)}.jpg`);
        
        // Determine frame time at ~1/3 duration, fallback to 1s
        const duration = await probeVideoDuration(videoPath);
        const targetTime = duration ? Math.max(0.5, Math.min(duration - 0.5, duration / 3)) : 1;
        
        const ok = await extractVideoFrame(videoPath, tmpOut, targetTime);
        if (!ok) {
        ...
        console.warn(`Video frame extraction failed for ${videoPath}:`, e.message);
        return false;
    } finally {
        releaseVideoWorker();
    }
...
async function generateThumbnail(mediaPath, thumbnailPath, mediaType) {
    if (mediaType === 'image') {
        ...
        // Try real frame extraction first (unless disabled)
...
<body>
    <div class="header">
        ...
        <div class="header-actions">
            ...
            <button class="header-btn" id="filterTagsBtn" onclick="toggleTagFilter()" style="display:none;" title="Filter by Tags">
                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
    ...
    <script>
        ...
        let tagCommandAvailable = false; // Will be set from API response
        ...
        async function loadAllTags() {
            ...
            // Show/hide tag button based on availability
            ...
            if (tagFilterBtn) {
                tagFilterBtn.style.display = (isMac && tagCommandAvailable) ? '' : 'none';
            }
        ...
        async function loadGallery() {
            ...
            // Show/hide tag button based on availability
            tagCommandAvailable = data.tagCommandAvailable;
            console.log("Tag availability:", data.tagCommandAvailable);
            if (data.tagCommandAvailable) {
                document.getElementById('tagBtn').style.display = 'flex';
            }
        ...
        async function openModal(media) {
            ...
            if (isMac && tagCommandAvailable) { tagBtn.style.display = 'flex'; } else { tagBtn.remove(); }
...
async function setupServer() {
    ...
    // macOS Finder tags support - uses functions from macos-tags.js module
    ...
    app.get('/api/gallery', async (req, res) => {
        try {
            ...
            res.json({ ...data, tagCommandAvailable });
        } catch (error) {
...
async function startServer() {
    try {
        // Check if macOS tag command is available
        tagCommandAvailable = await isTagCommandAvailable();
